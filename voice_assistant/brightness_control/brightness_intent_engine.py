"""
Brightness intent engine for parsing voice commands.
Uses fuzzy matching and regex to extract brightness control actions.
"""
...
class BrightnessIntentEngine:
    """
    Parses brightness control commands from recognized text.
    Supports fuzzy matching and numeric value extraction.
    """
    
    def __init__(self, commands_config: Dict[str, Any], noise_words: list):
        """
        Initialize brightness intent engine.
        ...
        # Extract brightness commands
        self.brightness_config = commands_config.get("brightness", {})
        self.actions = self.brightness_config.get("actions", {})
        
        # Precompile regex patterns for better performance
        self._compile_patterns()
        
        self.logger.info("BrightnessIntentEngine initialized")
        self.logger.info(f"Loaded {len(self.actions)} brightness actions")
    ...
    def _extract_numeric_value(self, text: str) -> Optional[int]:
        ...
        # We look for common number words in the text
    ...
    def parse_intent(self, text: str) -> Dict[str, Any]:
        ...
        if action:
            ...
            self.logger.log_intent_detected("brightness", action, validated_value, 1.0)
        ...
        if best_action and confidence >= 0.6:  # Fuzzy threshold
            ...
            self.logger.log_intent_detected("brightness", best_action, validated_value, confidence)
        ...
        # No intent detected
        self.logger.log_intent_detected("brightness", None, None, 0.0)
